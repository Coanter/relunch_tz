...
function HistoryPage() {
  ...
  const [menuHistory, setMenuHistory] = useState([
    {
      id: 1,
      name: "Меню для детей 3-5 лет.pdf",
      age: "3-5",
      season: "Осень",
    ...
    {
      id: 2,
      name: "Меню для детей 5-7 лет.pdf",
      age: "5-7",
      season: "Весна",
  ...
  const filteredData = menuHistory.filter((item) => {
    // Фильтрация по ключевому слову (ищем в названии)
    const matchKeyword = item.name.toLowerCase().includes(keyword.toLowerCase());
    // Фильтрация по возрасту (если выбран)
    const matchAge = age ? item.age === age : true;
    // Фильтрация по сезонности (если выбрана)
  ...
  const handleFilter = (e) => {
    e.preventDefault();
    // В данном примере фильтрация происходит «на лету» (см. filteredData),
    // поэтому отдельный обработчик может быть пустым
  };
